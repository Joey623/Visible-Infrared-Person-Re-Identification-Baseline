...
def main():
    ...
    if args.dataset == 'sysu':
        ...
        # testing set
        gallery_img, gallery_label, gallery_camid = process_gallery_sysu(args.data_root, mode=args.mode, shot=1,
                                                                         trial=0)
...
def test(gallery, query, model, epoch):
    ...
    # evaluation
    if args.dataset == 'sysu':
        cmc, mAP, mINP = eval_sysu(-distmat, query_label, gallery_label, query_camid, gallery_camid)
        cmc_global, mAP_global, mINP_global = eval_sysu(-distmat_global, query_label, gallery_label, query_camid, gallery_camid)
    elif args.dataset == 'regdb':
        cmc, mAP, mINP = eval_regdb(-distmat, query_label, gallery_label)
        cmc_global, mAP_global, mINP_global = eval_regdb(-distmat_global, query_label, gallery_label)
    else:
        raise Exception('invalid dataset name......')
    ...
    print('mINP_global: {:.2%}'.format(mINP_global))
