...
__all__ = ['vit_small_patch16_224', 'vit_base_patch16_224']
...
class Attention(nn.Module):
    def __init__(self,
        ...
        head_dim = dim // num_heads   # 除法，向下取整
        # NOTE scale factor was wrong in my original version, can set manually to be compat with prev weights
        self.scale = qk_scale or head_dim**-0.5  # 注意力机制里的根号dk

        self.qkv = nn.Linear(dim, dim * 3, bias=qkv_bias) # 构建qkv
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Linear(dim, dim)  # 映射
...
class TransReID(nn.Module):
    ...
    def __init__(self,
        ...
        if camera > 1 and view > 1:
            ...
            print('camera number is : {} and viewpoint number is : {}'.format(
                camera, view))
        ...
        if self.linear_block:
            print("using linear block")
            block_cls = LinearBlock
            img_size = to_2tuple(img_size)
            patch_size = to_2tuple(patch_size)
            stride_size_tuple = to_2tuple(stride_size)
            self.num_x = (img_size[1] -
                          patch_size[1]) // stride_size_tuple[1] + 1
            self.num_y = (img_size[0] -
                          patch_size[0]) // stride_size_tuple[0] + 1
            print('using stride: {}, and patch number is num_y{} * num_x{}'.
                  format(stride_size, self.num_y, self.num_x))
            num_patches = self.num_x * self.num_y
    ...
    def load_param(self, model_path):
        ...
        for k, v in param_dict.items():
            ...
            if 'patch_embed.proj.weight' in k and len(v.shape) < 4:
                ...
                v = resize_pos_embed(v, self.pos_embed, self.patch_embed.num_y,
            ...
            try:
                ...
                print(
                    'shape do not match in k :{}: param_dict{} vs self.state_dict(){}'
                    .format(k, v.shape,
...
def resize_pos_embed(posemb, posemb_new, hight, width):
    ...
    print(
        ...
        .format(posemb.shape, posemb_new.shape, hight, width))
...
def vit_base_patch16_224(img_size=(256, 128),
                                   stride_size=16,
                                   drop_rate=0.0,
                                   attn_drop_rate=0.0,
                                   drop_path_rate=0.1,
    ...
    model = TransReID(
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6),  sie_xishu=sie_xishu, local_feature=local_feature, linear_block=linear_block, **kwargs)
    return model


def vit_small_patch16_224(img_size=(256, 128),
                                    stride_size=16,
                                    drop_rate=0.,
                                    attn_drop_rate=0.,
                                    drop_path_rate=0.1,
